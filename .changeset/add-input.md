...
glow — a soft 4px ring at 25% opacity hugging the border (no offset) plus a
matching border, both colored by a dedicated `--input-ring` token (defaults to
`--primary`, so a host can recolor input focus per client project without
touching the button/primary color). When `aria-invalid` is set the glow and
border swap to `--destructive`, so the look never diverges from the screen-reader
semantics.
